...
/**
 * The as-of watermark a recon run evaluates: everything we had observed by then.
 * Derived from data, not from the clock, so re-running yields identical results.
 */
export async function currentWatermark(db: Db): Promise<Date | null> {
  const [row] = await db
    .select({
      // Raw aggregates bypass drizzle's column decoding; reuse observedAt's decoder.
      max: sql`max(${transactions.observedAt})`.mapWith(transactions.observedAt),
    })
    .from(transactions);
  return row?.max ?? null;
}

/** Current transaction versions observed by `asOf` — the run's reproducible input set. */
export async function loadCurrentTransactions(db: Db, asOf: Date) {
  return db
    .select()
    .from(transactions)
    .where(and(eq(transactions.isCurrent, true), lte(transactions.observedAt, asOf)));
}
