...
| Investigate with Claude: streaming loop, tools, verified citations (D38) | `apps/web/app/api/investigate/[exceptionId]/route.ts` (thin) + `apps/web/lib/investigate/*` (planner, tools, prompt, cite/present) + `components/case/investigate/*`; append-only store in `packages/db/src/services/investigation.ts`; api thread/budget endpoints in `apps/api/src/app.ts` | `db/src/test/investigation.test.ts`, api tests, and `lib/investigate/*.test.ts` + `RecordCite`/`Composer` component tests (streaming leans on the AI SDK) |
