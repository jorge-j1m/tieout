...
import { landAndFanOut } from "./land.js";

/**
 * Scheduled, windowed Stripe landing (D12): every run re-covers a lookback
...
export const landStripeTask = schedules.task({
  ...
  run: async (payload) => {
    ...
    if (!live) {
      logger.info(
        "live stripe landing not configured (STRIPE_LIVE_LANDING) — skipping scheduled poll",
      );
      return [];
    }
    return landAndFanOut(adapter, { to: payload.timestamp });
  },
});
